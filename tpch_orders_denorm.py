...
spark = SparkSession.builder.appName('tpch_3')\
...
def main(key_space, out_dir, order_keys):
    ...
    orders = read_table(key_space, 'orders_parts')\
    .where('orderkey in' + keys).select('orderkey','totalprice','part_names').orderBy('orderkey')
